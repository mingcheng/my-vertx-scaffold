...
public class ConsumerVerticle extends BaseVerticle {
  ...
  public void start(Future<Void> startFuture) throws Exception {
    ...
    consulClient
        .rxCatalogServiceNodes(GrpcServiceImpl.RPC_SERVER_NAME)
        .flatMap(
            result -> {
  ...
  private void greetFromRpcServer(String host, int port) {
    new AsyncResultSingle<Greet>(
            handler -> {
              ManagedChannel rpcChannel =
                  VertxChannelBuilder.forAddress(getVertx(), host, port).usePlaintext(true).build();

              GreetingServiceGrpc.GreetingServiceVertxStub stub =
                  GreetingServiceGrpc.newVertxStub(rpcChannel);
